...
function VehicleStackBarChart({ kode_prop, kode_kab }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:3001/api/1/${kode_prop}/${kode_kab}`);
    ...
    return (
        <TitleCard title="Jumlah Kendaraan berdasarkan Tipe">
            <Bar options={options} data={chartData} />
        </TitleCard>
    );
}

export default VehicleStackBarChart;
